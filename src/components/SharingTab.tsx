...
export default function SharingTab({ settings, initialSharedText = '' }: SharingTabProps) {
  ...
  const [enhancementTone, setEnhancementTone] = useState<'default' | 'poetic' | 'concise' | 'excited'>('default');
  const [originalBackup, setOriginalBackup] = useState('');
  const [enhancementError, setEnhancementError] = useState('');

  const handleEnhanceDescription = async () => {
    if (!description.trim()) {
      alert('يرجى إدخال وصف المنتج أولاً لتحسينه!');
      return;
    }
    setIsEnhancing(true);
    setEnhancementError('');
    if (!originalBackup) {
      setOriginalBackup(description);
    }
    ...
    try {
      const res = await fetch('/api/enhance-description', {
        ...
        body: JSON.stringify({ text: description, tone: enhancementTone }),
      });
      ...
      if (data.success && data.text) {
        ...
        throw new Error('لم تنجح معالجة النص بالذكاء الاصطناعي.');
      }
    } catch (err: any) {
      console.error('Enhance error:', err);
      setEnhancementError(err?.message || 'حدث خطأ أثناء الاتصال بخادم الذكاء الاصطناعي.');
    } finally {
              ...
              <div className="bg-orange-50/30 border border-orange-100 rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-gray-800 flex items-center gap-1.5">
                    ...
                    <span>المُصلح والكاتب الذكي بالذكاء الاصطناعي (Gemini)</span>
                  </span>
                  {originalBackup && (
                    <button
                      type="button"
                      onClick={handleRestoreOriginal}
                      className="text-[10px] text-gray-500 hover:text-[#F27D26] hover:underline font-bold transition cursor-pointer"
                    >
                      تراجع واستعادة الأصلي
                    </button>
                  )}
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  {/* Tone Selectors */}
                  <div className="grid grid-cols-4 gap-1 bg-white p-1 rounded-xl border border-gray-150 sm:flex-1">
                    {(['default', 'poetic', 'concise', 'excited'] as const).map((tone) => {
                      const label = {
                        default: 'تسويقي',
                        poetic: 'شاعري',
                        concise: 'مختصر',
                      ...
                      const isToneSelected = enhancementTone === tone;
                      return (
                        <button
                          key={tone}
                          type="button"
                          onClick={() => setEnhancementTone(tone)}
                          className={`py-1.5 px-1 text-[10px] font-bold rounded-lg transition text-center cursor-pointer sm:flex-1 ${
                            isToneSelected
                  ...
                  <button
                    ...
                    className="sm:w-auto bg-[#F27D26] hover:bg-[#d96a1a] disabled:opacity-50 text-white font-bold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition cursor-pointer shadow-sm"
                  >
                    {isEnhancing ? (
                      <>
                  ...
                  {filteredImages.map(img => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isSelected && loadedFiles[img.id]?.error && (
                          <div className="absolute inset-0 bg-red-500/10 backdrop-blur-[1px] border border-red-500 flex flex-col items-center justify-center text-red-600 p-2 text-center gap-1">
                            <span className="text-[10px] font-bold">⚠️ خطأ في التجهيز</span>
                            <span className="text-[8px] font-medium leading-tight max-h-[36px] overflow-y-auto px-1">
                              {loadedFiles[img.id]?.error}
                            </span>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              className="mt-1 px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white rounded text-[8px] font-bold shadow-sm transition-all"
                            >
                              إعادة المحاولة
                            </button>
                          </div>
